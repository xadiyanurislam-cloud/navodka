...
def keep_candidates(cands, verdicts):
    """Какие из кандидатов попадут в карточку и с какой подписью.

    Подтверждённые почтовым сервером адреса — находка, и их можно
    оставить все: сервер сказал «такой ящик есть». Если же не
    подтвердился ни один — проверка выключена, домен принимает что
    угодно или молчит, — остаётся ровно одна строка. Пяти «почт
    руководителя» на одном домене не бывает, а человек, который
    обзванивает базу, разбирает их всерьёз и теряет на этом время.
    """
    cands = [(a, c) for a, c in (cands or []) if verdicts.get(a) != "bad"]
    if not cands:
        return [], ""
    ok = [(a, c) for a, c in cands if verdicts.get(a) == "ok"]
    if ok:
        return ok, "подтверждён почтовым сервером"
    if verdicts.get(cands[0][0]) == "catch_all":
        return cands[:1], "домен принимает любой адрес — это догадка"
    return cands[:1], "выведен по схеме домена"
