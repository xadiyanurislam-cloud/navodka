    ...
    if err:
        ...
        for t in net.hh_transports():
            ...
            try:
                ...
                # Причина обязана быть видна: это экран диагностики, и
                # «не дозвонился» без объяснения — ровно то, за чем сюда
                # заходят. Раньше причина отбрасывалась вместе с
                # исключением, и три способа связи отличались только
                # названием.
                tried.append("%s — не дозвонился: %s"
                             % (t.name, str(e)[:120]))
                continue
